...
class OpeningExplorerDialog(QDialog):
    ...
    def on_opening_selected(self, current_item, previous_item):
        ...
        if isinstance(opening_obj, chess.openings.Opening):
            ...
            self.mainline_display.setText(" ".join(m.uci() for m in opening_obj.mainline()))
...
class ClickableBoardWidget(QSvgWidget):
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            file_idx_visual, rank_idx_visual_from_top = int(x / square_width), int(y / square_height)
            if 0 <= file_idx_visual < 8 and 0 <= rank_idx_visual_from_top < 8:
                true_rank = (7 - rank_idx_visual_from_top) if self.player_color == chess.WHITE else rank_idx_visual_from_top
                true_file = file_idx_visual if self.player_color == chess.WHITE else (7 - file_idx_visual)
...
class MainWindow(QMainWindow):
    ...
    def _create_menu(self):
        ...
        play_as_white_action.triggered.connect(lambda: self.new_standard_game_as_color(chess.WHITE))
        new_game_menu.addAction(play_as_white_action)

        play_as_black_action = QAction("Play as Black", self)
        play_as_black_action.triggered.connect(lambda: self.new_standard_game_as_color(chess.BLACK))
        ...
        new_std_game_direct_action.triggered.connect(self.new_standard_game) # new_standard_game will call ..._as_color
    ...
    def new_standard_game_as_color(self, player_chosen_color: bool):
        ...
        logger.info(f"MainWindow: Starting new standard game as {'White' if player_chosen_color == chess.WHITE else 'Black'}.")
        ...
        self.status_label.setText(f"New game. Player is {color_name}. {'Your turn.' if self.player_color == chess.WHITE else 'White (AI) to move.'}")
        ...
        if self.player_color == chess.BLACK and self.board.turn == chess.WHITE:
            ...
            QTimer.singleShot(150, self.request_engine_move_clicked) # Slightly longer delay for UI
    ...
    def update_board_display(self):
        ...
        fill_dict = {self.selected_square: "#cc0000aa"} if self.selected_square is not None else {}
        svg_bytes = chess.svg.board(self.board, orientation=self.player_color, lastmove=last_move, fill=fill_dict, size=400).encode("UTF-8")
    ...
    def new_from_fen(self):
        ...
        if ok and fen: self.load_fen(fen)

    def load_fen_from_file(self):
        path, _ = QFileDialog.getOpenFileName(self, "Open FEN File", "", "FEN (*.fen);;All (*)")
        if path:
            try:
                with open(path, "r", encoding="utf-8") as f: self.load_fen(f.readline().strip())
            except Exception as e: QMessageBox.warning(self, "File Error", str(e))

    def toggle_analysis_clicked(self):
        if not self.engine_worker or not self.engine_worker.engine:
            QMessageBox.warning(self, "Engine Not Ready", "Engine not available."); return
        ...
        if state == EngineState.ANALYZING:
            ...
            QMessageBox.information(self, "Engine Busy", f"Engine is {state.name}. Cannot toggle analysis.")
        self.update_ui_from_engine_state()

    def request_engine_move_clicked(self):
        if self.is_in_review_mode:
            QMessageBox.information(self, "Review Mode", "Request engine move disabled during PGN review.")
            return
        if not self.engine_worker or not self.engine_worker.engine or self.board.is_game_over() or self.engine_worker.get_state() != EngineState.IDLE:
            QMessageBox.warning(self, "Engine Not Ready", "Engine not ready or game over."); return
        self.status_label.setText(f"Status: Engine ({self.engine_path}) is thinking...")
        self.is_engine_thinking_ui_flag = True; self.update_ui_elements(); QApplication.processEvents()
        move = self.engine_worker.request_best_move(self.board.copy(), 1.0)
        self.is_engine_thinking_ui_flag = False
        if move: self.handle_engine_move(move)
        else: self.status_label.setText("Status: Engine failed to find a move.")
        self.update_ui_elements(); self.update_ui_from_engine_state()

    def handle_engine_move(self, move: chess.Move):
        if move and move in self.board.legal_moves:
            self.board.push(move); self.update_board_display()
            turn = "White" if self.board.turn == chess.WHITE else "Black"
            self.status_label.setText(f"Status: Engine played {move.uci()}. {turn}'s turn.")
            self.clock.switch()
            if self.board.is_game_over(): self.clock.stop(); self.handle_game_over()
            else: self.clock.start(self.board.turn)
        elif move: QMessageBox.critical(self, "Illegal Engine Move", f"Engine suggested: {move.uci()}")
        self.update_ui_elements()

    def handle_engine_analysis(self, data: dict | None):
        if not data: self.analysis_display.clear(); return
        score, pv = data.get("score"), data.get("pv")
        s = f"{score.relative.score(mate_score=10000)/100.0:.2f}" if score and not score.is_mate() else f"Mate in {score.mate()}" if score else "N/A"
        pv_str = " ".join(m.uci() for m in pv) if pv else ""
        self.analysis_display.setText(f"Score: {s} | PV: {pv_str} | Depth: {data.get('depth', '-')}")

    def update_ui_from_engine_state(self):
        if not self.engine_worker:
            if hasattr(self, 'status_label'): self.status_label.setText("Status: Engine not available.")
            return
        state = self.engine_worker.get_state()
        ...
        if not self.is_in_review_mode and not self.practice_opening_mainline:
            if state == EngineState.IDLE: self.status_label.setText(f"Status: Engine Idle. {'White' if self.board.turn == chess.WHITE else 'Black'}'s turn.")
            elif state == EngineState.ANALYZING: self.status_label.setText("Status: Engine Analyzing...")
            elif state == EngineState.THINKING: self.status_label.setText("Status: Engine Thinking...")
            elif state == EngineState.SHUTDOWN: self.status_label.setText("Status: Engine Offline.")
    ...
    def update_ui_elements(self):
        ...
        if hasattr(self, 'start_analysis_button'):
            self.start_analysis_button.setText("Stop Analysis" if state == EngineState.ANALYZING else "Start Analysis")
            self.start_analysis_button.setEnabled(not in_special_mode and (state == EngineState.ANALYZING or \
                (state == EngineState.IDLE and not game_over and not self.is_engine_thinking_ui_flag)))

        if hasattr(self, 'request_move_button'):
            self.request_move_button.setEnabled(not in_special_mode and state == EngineState.IDLE and \
                not game_over and not self.analysis_is_on_ui_flag and not self.is_engine_thinking_ui_flag)
    ...
    def handle_square_click(self, sq_idx: int):
        ...
        if self.board.turn != self.player_color:
            if self.selected_square is not None: self.selected_square = None; self.update_board_display()
            return
        piece = self.board.piece_at(sq_idx)
        if self.selected_square is None:
            if piece and piece.color == self.player_color: self.selected_square = sq_idx; self.update_board_display()
        else:
            from_sq, to_sq = self.selected_square, sq_idx
            self.selected_square = None
            if from_sq == to_sq: self.update_board_display(); return
            promo = None
            ...
            if moving_piece and moving_piece.piece_type == chess.PAWN and chess.square_rank(to_sq) in [0,7]:
                ...
                item, ok = QInputDialog.getItem(self, "Promotion", "Promote to:", items, 0, False)
                if ok and item: promo = {"Queen": chess.QUEEN, "Rook": chess.ROOK, "Bishop": chess.BISHOP, "Knight": chess.KNIGHT}[item]
                else: self.selected_square = from_sq; self.update_board_display(); return
            move = chess.Move(from_sq, to_sq, promotion=promo)
            if move in self.board.legal_moves:
                self.board.push(move); self.update_board_display(); self.clock.switch()
                if self.board.is_game_over(): self.handle_game_over()
                else:
                    self.clock.start(self.board.turn)
                    if self.board.turn != self.player_color and self.engine_worker and self.engine_worker.engine:
                        # AI's turn
                ...
                if piece is not None and piece.color == self.player_color and self.board.turn == self.player_color:
                    self.selected_square = to_sq # Select the new piece
                    logger.info(f"MainWindow: Illegal move, but target square has own piece. Selected {piece} at {chess.square_name(to_sq)}")
    ...
    def handle_game_over(self):
        ...
        msg = "Game over."
        if outcome:
            if outcome.termination == chess.Termination.CHECKMATE: msg = f"Checkmate! {'Black' if outcome.winner == chess.BLACK else 'White'} wins."
            elif outcome.termination == chess.Termination.STALEMATE: msg = "Stalemate! Draw."
            else: msg = f"Game over: {outcome.result()}"
        elif self.board.is_checkmate(): msg = f"Checkmate! {'Black' if self.board.turn == chess.WHITE else 'White'} wins."
        QMessageBox.information(self, "Game Over", msg)
        if hasattr(self, 'status_label'): self.status_label.setText(f"Status: {msg}")
        if self.engine_worker and self.engine_worker.get_state() == EngineState.ANALYZING: self.engine_worker.stop_analysis()
        self.end_opening_practice_mode()
        self.setup_pgn_review_mode(False)
    ...
    def handle_setup_opening_for_practice(self, opening: chess.openings.Opening):
        ...
        self.player_color = opening.board.turn # Practice from the perspective of whose turn it is in opening
        if hasattr(self.board_display, 'player_color'):
            self.board_display.player_color = self.player_color

        self.end_opening_practice_mode(); self.setup_pgn_review_mode(False)

        if self.engine_worker and self.engine_worker.get_state() == EngineState.ANALYZING:
            self.engine_worker.stop_analysis()

        self.board = opening.board.copy() # Set board to the starting position of the opening
        ...
        if hasattr(self, 'prev_mainline_move_button'): self.prev_mainline_move_button.setVisible(True)
        if hasattr(self, 'next_mainline_move_button'): self.next_mainline_move_button.setVisible(True)
    ...
    def show_previous_mainline_move(self):
        if not self.practice_opening_mainline or self.current_practice_move_index < 0: return
        self.board.pop(); self.current_practice_move_index -= 1
        self.update_board_display(); self.update_practice_buttons_state()
        self.status_label.setText(f"Status: Practice: M {self.current_practice_move_index + 1}/{len(self.practice_opening_mainline)}")

    def show_next_mainline_move(self):
        if not self.practice_opening_mainline or self.current_practice_move_index >= len(self.practice_opening_mainline) - 1: return
        self.current_practice_move_index += 1
        self.board.push(self.practice_opening_mainline[self.current_practice_move_index])
        self.update_board_display(); self.update_practice_buttons_state()
        self.status_label.setText(f"Status: Practice: M {self.current_practice_move_index + 1}/{len(self.practice_opening_mainline)}")

    def update_practice_buttons_state(self):
        is_active = bool(self.practice_opening_mainline)
        if hasattr(self, 'prev_mainline_move_button'): self.prev_mainline_move_button.setEnabled(is_active and self.current_practice_move_index >= 0)
        if hasattr(self, 'next_mainline_move_button'): self.next_mainline_move_button.setEnabled(is_active and self.current_practice_move_index < len(self.practice_opening_mainline) - 1)

    def end_opening_practice_mode(self):
        if not self.practice_opening_mainline and self.current_practice_move_index == -1: return # Not in practice mode
        self.practice_opening_mainline = []; self.current_practice_move_index = -1
        if hasattr(self, 'prev_mainline_move_button'): self.prev_mainline_move_button.setVisible(False)
        if hasattr(self, 'next_mainline_move_button'): self.next_mainline_move_button.setVisible(False)
    ...
    def setup_pgn_review_mode(self, enable: bool):
        ...
        if enable:
            self.end_opening_practice_mode()
            if hasattr(self,'prev_pgn_move_button'): self.prev_pgn_move_button.setVisible(True)
            if hasattr(self,'next_pgn_move_button'): self.next_pgn_move_button.setVisible(True)
            self.update_pgn_review_buttons_state()
            self.is_in_review_mode = True
        else:
            if hasattr(self,'prev_pgn_move_button'): self.prev_pgn_move_button.setVisible(False)
            if hasattr(self,'next_pgn_move_button'): self.next_pgn_move_button.setVisible(False)
            self.is_in_review_mode = False
            self.loaded_pgn_moves = []
            self.current_loaded_pgn_move_index = -1
    ...
    def load_game_from_pgn(self):
        path, _ = QFileDialog.getOpenFileName(self, "Load PGN", "", "PGN (*.pgn);;All (*)")
        if not path: return
        self.end_opening_practice_mode(); self.setup_pgn_review_mode(False) # Reset modes
        if self.engine_worker and self.engine_worker.get_state() == EngineState.ANALYZING: self.engine_worker.stop_analysis()
        try:
            with open(path, "r", encoding="utf-8") as f: game = chess.pgn.read_game(f)
            if game is None: QMessageBox.warning(self, "Load PGN Error", "Invalid PGN."); return
            ...
            self.board.starting_fen = self.board.fen() # This is the effective starting FEN

            self.loaded_pgn_moves = list(game.mainline_moves())
            self.current_loaded_pgn_move_index = -1 # At the position *before* the first PGN move

            self.update_board_display()
            self.status_label.setText(f"PGN loaded: {os.path.basename(path)}. Board at start. Use PGN nav buttons.")
            self.clock.reset(); self.clock.stop(); self.update_clock_labels(self.clock.white_time, self.clock.black_time)
            self.selected_square = None
    ...
    def pgn_show_previous_move(self):
        if not self.is_in_review_mode or self.current_loaded_pgn_move_index < 0: return
        self.board.pop(); self.current_loaded_pgn_move_index -= 1; self.selected_square = None
        self.update_board_display(); self.update_pgn_review_buttons_state()
        current_move_display = self.current_loaded_pgn_move_index + 1
        self.status_label.setText(f"PGN: Move {current_move_display}/{len(self.loaded_pgn_moves)}")

    def pgn_show_next_move(self):
        if not self.is_in_review_mode or self.current_loaded_pgn_move_index >= len(self.loaded_pgn_moves) - 1: return
        self.current_loaded_pgn_move_index += 1
        move = self.loaded_pgn_moves[self.current_loaded_pgn_move_index]
        self.board.push(move); self.selected_square = None
        self.update_board_display(); self.update_pgn_review_buttons_state()
        current_move_display = self.current_loaded_pgn_move_index + 1
        self.status_label.setText(f"PGN: Move {current_move_display}/{len(self.loaded_pgn_moves)}. Played {move.uci()}")

    def update_pgn_review_buttons_state(self):
        is_active = self.is_in_review_mode and bool(self.loaded_pgn_moves)
        if hasattr(self,'prev_pgn_move_button'): self.prev_pgn_move_button.setEnabled(is_active and self.current_loaded_pgn_move_index >= 0)
        if hasattr(self,'next_pgn_move_button'): self.next_pgn_move_button.setEnabled(is_active and self.current_loaded_pgn_move_index < len(self.loaded_pgn_moves) - 1)

    def update_clock_labels(self, white_seconds: int, black_seconds: int):
        def fmt_time(s): return f"{s//60:02d}:{s%60:02d}"
    ...
    def update_opening_info(self):
        try: opening_name = chess.polyglot.opening_name(self.board)
        except: opening_name = "Unknown"
        ...
        except: self.opening_line_label.setText("ECO: -")
        moves = fetch_lichess_moves(self.board)
        if moves:
            total = sum(m.get("white",0)+m.get("draws",0)+m.get("black",0) for m in moves)
            parts = [f"{m.get('san',m.get('uci',''))} ({100*(m.get('white',0)+m.get('draws',0)+m.get('black',0))/total:.1f}%)" for m in moves[:3] if total]
            self.lichess_moves_label.setText("Lichess: " + ", ".join(parts))
        else: self.lichess_moves_label.setText("Lichess: n/a")

    def save_game_as_pgn(self):
        if not self.board.move_stack: QMessageBox.information(self, "No Moves", "No moves to save."); return
        path, _ = QFileDialog.getSaveFileName(self, "Save PGN", "game.pgn", "PGN (*.pgn);;All (*)")
        if not path: return
        try:
            game = chess.pgn.Game()
            game.headers["Event"] = "Casual Game"; game.headers["Site"] = "Local"
            game.headers["Date"] = datetime.datetime.now().strftime("%Y.%m.%d")
            game.headers["Round"] = "?"; game.headers["White"] = "PlayerWhite"; game.headers["Black"] = "PlayerBlack"
            if self.board.chess960: game.headers["Variant"] = "Chess960"
            ...
            pgn_string = game.accept(chess.pgn.StringExporter(headers=True, variations=True, comments=True))
            with open(path, "w", encoding="utf-8") as f: f.write(pgn_string)
            QMessageBox.information(self, "Save Successful", f"Game saved to:\n{path}")
        except Exception as e: QMessageBox.critical(self, "Save Error", f"Could not save PGN: {e}")

    def closeEvent(self, event):
        if self.engine_worker:
            if self.engine_worker.get_state() == EngineState.ANALYZING: self.engine_worker.stop_analysis()
