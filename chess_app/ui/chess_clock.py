...
class ChessClock(QObject):
    ...
    def __init__(self, initial_seconds: int = 300):
        ...
        self.active_color: bool | None = None  # Stores whose clock is currently running (chess.WHITE or chess.BLACK)

        self._timer: QTimer = QTimer(self)
        self._timer.setInterval(1000) # Timer ticks every 1000 milliseconds (1 second)
        self._timer.timeout.connect(self._tick) # Connect timeout signal to the _tick method
    ...
    def switch(self):
        ...
        if self.active_color is not None:
            self.active_color = not self.active_color # Toggle between chess.WHITE (True) and chess.BLACK (False)
    ...
    def _tick(self):
        ...
        """
        if self.active_color is None: # Should not happen if timer is active, but as a safeguard
            return
        ...
        self.time_changed.emit(self.white_seconds, self.black_seconds) # Notify listeners of the time update
