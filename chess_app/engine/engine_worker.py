...
class EngineWorker:
    ...
    def __init__(self, engine_path: str):
        ...
        self.worker_thread: threading.Thread = threading.Thread(
            target=self.run, name="EngineWorkerThread", daemon=True
        )
        self.worker_thread.start()
        logger.info(
            f"EngineWorker initialized for {engine_path} and worker thread started."
        )
    ...
    def _initialize_engine(self) -> bool:
        ...
        try:
            logger.info(
                f"Engine _initialize_engine: Initializing chess engine from path: {self.engine_path}"
            )
            self.engine = chess.engine.SimpleEngine.popen_uci(self.engine_path)
            self.analysis_stopped_event.set()  # Initialize as set, as no analysis is running.
            logger.info(
                "Engine _initialize_engine: Chess engine initialized successfully."
            )
            return True
        except Exception as e:
            logger.error(
                f"Engine _initialize_engine: Failed to initialize chess engine: {e}",
                exc_info=True,
            )
    ...
    def run(self):
        ...
        if not self._initialize_engine():
            logger.error(
                "Engine run: Engine initialization failed. Worker thread stopping."
            )
        ...
        while self.state != EngineState.SHUTDOWN:
            try:
                ...
                args = (
                    args_tuple if isinstance(args_tuple, tuple) else (args_tuple,)
                )  # Ensure args is a tuple

                logger.info(
                    f"Engine run: Received command: {command} with args: {args if args else ''}"
                )
                ...
                logger.error(
                    f"Engine run: Unexpected error in command loop: {e}", exc_info=True
                )
    ...
    def _handle_start_analysis(self, board: chess.Board):
        ...
        if self.state != EngineState.IDLE:
            logger.warning(
                f"Engine _handle_start_analysis: Engine not IDLE (state: {self.state}). Cannot start analysis."
            )
            return

        if (
            not self.engine
        ):  # Should not happen if initialized correctly and not shut down.
            logger.error(
                "Engine _handle_start_analysis: Engine not initialized. Cannot start analysis."
            )
            return
        ...
        try:
            ...
            logger.warning(
                f"Engine _handle_start_analysis: Failed to configure UCI_Chess960 option: {e}"
            )
        ...
        try:
            ...
            with self.engine.analysis(board, stop=self.stop_event) as analysis_result:
                ...
                for info in self.analysis_info:
                    if self.stop_event.is_set():  # Check if a stop has been requested.
                        logger.info(
                            "Engine _handle_start_analysis: Stop event detected during analysis. Terminating analysis stream."
                        )
                        break
                    self.latest_analysis_info = (
                        info  # Store the latest piece of analysis.
                    )
                    ...
                    if score is not None and pv_moves is not None:
                        logger.debug(
                            f"Engine _handle_start_analysis: Analysis - Score: {score}, PV: {[str(m) for m in pv_moves]}"
                        )

        except chess.engine.EngineTerminatedError:
            logger.error(
                "Engine _handle_start_analysis: Engine terminated unexpectedly during analysis.",
                exc_info=True,
            )
            ...
            logger.error(
                f"Engine _handle_start_analysis: An error occurred during analysis: {e}",
                exc_info=True,
            )
        finally:
            ...
            logger.info(
                "Engine _handle_start_analysis: Engine state set to IDLE and analysis_stopped_event is set."
            )
    ...
    def _handle_stop_analysis(self):
        ...
        if self.state != EngineState.ANALYZING:
            logger.warning(
                f"Engine _handle_stop_analysis: Engine not in ANALYZING state (current: {self.state}). Cannot stop analysis."
            )
        ...
        logger.info(
            "Engine _handle_stop_analysis: Processing STOP_ANALYSIS command. Setting stop_event."
        )
    ...
    def _handle_find_best_move(
        ...
        if self.state != EngineState.IDLE:
            logger.warning(
                f"Engine _handle_find_best_move: Engine not IDLE (state: {self.state}). Cannot find best move now."
            )
            if result_queue:
        ...
        if not self.engine:
            logger.error(
                "Engine _handle_find_best_move: Engine not initialized. Cannot find best move."
            )
            if result_queue:
        ...
        logger.info(
            f"Engine _handle_find_best_move: Changing state to THINKING for board: {board.fen()} with time limit: {time_limit}s."
        )
        try:
            self.engine.configure({"UCI_Chess960": board.chess960})
        except Exception as e:
            logger.warning(
                f"Engine _handle_find_best_move: Failed to configure UCI_Chess960 option: {e}"
            )
        ...
        try:
            ...
            logger.error(
                "Engine _handle_find_best_move: Engine terminated unexpectedly while finding best move.",
                exc_info=True,
            )
            self._handle_engine_failure()  # Handle critical engine failure.
        except Exception as e:
            logger.error(
                f"Engine _handle_find_best_move: Error finding best move: {e}",
                exc_info=True,
            )
        finally:
            self.state = (
                EngineState.IDLE
            )  # Return to IDLE state once finished or if an error occurred.
            logger.info(
                "Engine _handle_find_best_move: Finished finding best move. Engine state set to IDLE."
            )
            if result_queue:
                result_queue.put(
                    best_move
                )  # Send the result (or None if error) back to the requester.

    def _handle_engine_failure(self):
        """Handles critical engine failures, like unexpected termination."""
        logger.critical(
            "Engine _handle_engine_failure: Critical engine error detected. Attempting to handle."
        )
        # Log with ERROR as it's a significant problem affecting functionality. CRITICAL for the initial detection.
        logger.error(
            "Engine _handle_engine_failure: Setting state to SHUTDOWN and attempting to close engine."
        )
        ...
        if self.engine:
            try:
                ...
                logger.error(
                    f"Engine _handle_engine_failure: Exception during emergency engine quit: {e}",
                    exc_info=True,
                )
    ...
    def _handle_quit(self):
        ...
        if self.state == EngineState.ANALYZING:
            logger.info(
                "Engine _handle_quit: Quit requested during analysis. Signaling analysis to stop."
            )
            ...
            if not self.analysis_stopped_event.wait(
                timeout=5.0
            ):  # Timeout to prevent indefinite blocking.
                logger.warning(
                    "Engine _handle_quit: Timeout waiting for analysis to stop during quit. Proceeding with engine shutdown."
                )
            else:
    ...
    def _cleanup_engine(self):
        ...
        if self.engine:
            try:
                ...
                logger.info(
                    "Engine _cleanup_engine: Engine already terminated when attempting cleanup quit."
                )
            except Exception as e:
                logger.error(
                    f"Engine _cleanup_engine: Error quitting engine during cleanup: {e}",
                    exc_info=True,
                )
            self.engine = None
    ...
    def start_analysis(self, board: chess.Board):
        ...
        if not isinstance(board, chess.Board):
            logger.error(
                f"Engine start_analysis: Invalid board type provided: {type(board)}"
            )
            return
    ...
    def stop_analysis(self) -> bool:
        ...
        if current_engine_state != EngineState.ANALYZING:
            logger.warning(
                f"Engine stop_analysis: Engine not in ANALYZING state (current: {current_engine_state}). Stop request ignored or analysis already stopped."
            )
        ...
        logger.info(
            "Engine stop_analysis: STOP_ANALYSIS command queued. Waiting for analysis_stopped_event."
        )
        ...
        if not self.analysis_stopped_event.wait(timeout=10.0):
            logger.error(
                "Engine stop_analysis: Timeout waiting for analysis_stopped_event. Analysis may not have stopped cleanly in the engine process."
            )
            ...
            logger.info(
                "Engine stop_analysis: Analysis confirmed stopped (analysis_stopped_event was set). Engine should be IDLE."
            )
    ...
    def request_best_move(
        ...
        if self.get_state() != EngineState.IDLE:
            logger.warning(
                f"Engine request_best_move: Engine not IDLE (state: {self.get_state()}). Best move request rejected."
            )
            return None
        ...
        result_queue: queue.Queue = (
            queue.Queue()
        )  # Queue for receiving the result from the worker thread.
        self.command_queue.put(("FIND_BEST_MOVE", (board, time_limit, result_queue)))

        logger.info(
            "Engine request_best_move: FIND_BEST_MOVE command queued. Waiting for result."
        )
        try:
            ...
            if best_move:
                logger.info(
                    f"Engine request_best_move: Best move received from worker: {best_move}"
                )
            else:
                logger.warning(
                    "Engine request_best_move: Received None or no move from worker."
                )
            return best_move
        except queue.Empty:
            logger.error(
                f"Engine request_best_move: Timeout waiting for best move result from worker thread (limit: {time_limit + 5.0}s)."
            )
            return None
        except Exception as e:  # Should not happen if worker thread handles its errors.
            logger.error(
                f"Engine request_best_move: Exception retrieving best move result: {e}",
                exc_info=True,
            )
            return None
    ...
    def quit_engine(self):
        ...
        if self.state != EngineState.SHUTDOWN:
            ...
            try:
                ...
                if self.worker_thread.is_alive():
                    logger.error(
                        "Engine quit_engine: Worker thread did not terminate cleanly after QUIT command and join timeout."
                    )
                else:
                    logger.info(
                        "Engine quit_engine: Worker thread terminated successfully."
                    )
            except Exception as e:  # Catch potential errors during join, though rare.
                logger.error(
                    f"Engine quit_engine: Exception during worker_thread.join: {e}",
                    exc_info=True,
                )
        else:
        ...
        if hasattr(self, "worker_thread") and self.worker_thread.is_alive():
            logger.warning(
                "Engine quit_engine: Worker thread is still alive after quit sequence. This might indicate an issue."
            )
...
if __name__ == "__main__":
    ...
    main_logger = logging.getLogger(
        __name__
    )  # Use __name__ for the logger in this example context.
    ...
    try:
        ...
        main_logger.critical(
            f"Example Main: Failed to instantiate EngineWorker: {e}", exc_info=True
        )
    ...
    while (
        worker.get_state() != EngineState.IDLE
        and worker.get_state() != EngineState.SHUTDOWN
    ):
        main_logger.info(
            f"Example Main: Waiting for engine to initialize... current state: {worker.get_state()}"
        )
    ...
    main_logger.info(
        f"Example Main: Engine initialized. Current state: {worker.get_state()}"
    )
    ...
    if worker.get_state() == EngineState.IDLE:
        ...
        main_logger.info(
            f"Example Main: Requesting to start analysis on FEN: {current_board.fen()}"
        )
        ...
        if worker.get_state() == EngineState.ANALYZING:
            main_logger.info(
                "Example Main: Analysis started. Let it run for a few seconds..."
            )
            time.sleep(3)  # Let analysis run
            latest_info = worker.get_latest_analysis_info()
            main_logger.info(
                f"Example Main: Latest analysis info during run: {latest_info}"
            )
            main_logger.info("Example Main: Requesting to stop analysis.")
            if worker.stop_analysis():  # stop_analysis is blocking
                main_logger.info(
                    f"Example Main: Analysis stopped successfully. Engine state: {worker.get_state()}"
                )
            else:
                main_logger.warning(
                    f"Example Main: Analysis stop timed out. Engine state: {worker.get_state()}"
                )
        else:
            main_logger.warning(
                f"Example Main: Failed to start analysis. State: {worker.get_state()}"
            )
    ...
    if worker.get_state() == EngineState.IDLE:
        ...
        current_board = chess.Board()
        main_logger.info(
            f"Example Main: Requesting best move for FEN: {current_board.fen()} (Time limit: 1s)"
        )
        move = worker.request_best_move(
            current_board.copy(), time_limit=1.0
        )  # Blocking
        if move:
        ...
        main_logger.info(
            f"Example Main: Engine state after best move: {worker.get_state()}"
        )
    else:
        main_logger.warning(
            f"Example Main: Engine not IDLE (state: {worker.get_state()}), skipping best move test."
        )
    ...
    if worker.get_state() == EngineState.IDLE:
        main_logger.info(
            "--- Example Main: Test Case 3: Find move while analyzing (expected failure/rejection) ---"
        )
        current_board = chess.Board()
        worker.start_analysis(current_board.copy())
        ...
        if worker.get_state() == EngineState.ANALYZING:
            main_logger.info(
                "Example Main: Analysis started. Now attempting to request best move (should be rejected)."
            )
            move = worker.request_best_move(current_board.copy(), time_limit=0.5)
            if move is None:
                main_logger.info(
                    "Example Main: Best move request was correctly rejected or failed as engine was busy."
                )
            else:
                main_logger.error(
                    f"Example Main: Best move was unexpectedly processed: {move}"
                )
            main_logger.info("Example Main: Stopping analysis for Test Case 3.")
            worker.stop_analysis()
        else:
            main_logger.warning(
                "Example Main: Could not start analysis for Test Case 3"
            )

    main_logger.info(
        f"Example Main: Final engine state before explicit quit: {worker.get_state()}"
    )
    # --- Quit Engine ---
    main_logger.info("--- Example Main: Quitting Engine ---")
    worker.quit_engine()
    main_logger.info(
        f"Example Main: Engine quit requested. Final state from get_state: {worker.get_state()}"
    )
