    ...
    def __enter__(self):
        ...
        def analysis_generator():
            ...
            while not self.worker_stop_event.is_set() and count < 5:  # Limit iterations
                yield {
                    "score": chess_stub.engine.PovScore(
                        chess_stub.engine.Cp(10), chess_stub.WHITE
                    ),
                    "pv": [chess_stub.Move.from_uci("e2e4")],
                }
...
class DummyEngine:
    def __init__(self, path="dummy_engine_path"):
        ...
        self.is_alive_mock = MagicMock(
            return_value=True
        )  # To mock internal engine process checks
...
engine_module.SimpleEngine.popen_uci = MagicMock(
    return_value=DummyEngine("global_dummy")
)
...
engine_module.PovScore = lambda score_type, turn: {
...
engine_module.Limit = lambda time=None, depth=None, nodes=None: {
    "time": time,
    "depth": depth,
    "nodes": nodes,
}
...
class TestEngineWorker(unittest.TestCase):
    ...
    def setUp(self):
        ...
        engine_module.SimpleEngine.popen_uci = MagicMock(
            return_value=DummyEngine(f"dummy_for_{self.id()}")
        )
    ...
    def wait_for_condition(
        self, condition_callable, timeout=3.0, interval=0.01, description="condition"
    ):
    ...
    def wait_for_engine_state(self, worker, expected_state, timeout=2.0):
        self.wait_for_condition(
            lambda: worker.get_state() == expected_state,
            timeout=timeout,
            description=f"engine state to be {expected_state}",
        )

    def wait_for_engine_init(self, worker, timeout=2.0):
        self.wait_for_condition(
            lambda: worker.engine is not None
            and worker.get_state() == EngineState.IDLE,
            timeout=timeout,
            description="engine to initialize and be IDLE",
        )
    ...
    def test_engine_initialization_and_quit(self, mock_popen_uci):
        ...
        self.assertEqual(
            worker.get_state(), EngineState.IDLE, "Engine should be IDLE after init."
        )
        ...
        self.assertFalse(
            ...
            "Worker thread should not be alive after quit.",
        )
    ...
    def test_engine_worker_start_stop_analysis(self, mock_popen_uci):
        ...
        self.wait_for_engine_state(
            worker, EngineState.ANALYZING, timeout=0.5
        )  # Analysis starts quickly
        ...
        self.assertFalse(
            ...
            "Stop event should be clear before stop_analysis",
        )
        ...
        self.assertTrue(
            worker.analysis_stopped_event.is_set(),
            "analysis_stopped_event should be set after analysis stops.",
        )
    ...
    @patch(
        ...
        side_effect=Exception("Simulated engine start failure"),
    )
    def test_engine_initialization_failure(self, mock_popen_uci_failure):
        ...
        self.assertIsNone(
            worker.engine, "Engine should be None after initialization failure."
        )
        self.assertEqual(
            worker.get_state(), EngineState.SHUTDOWN, "Engine state should be SHUTDOWN."
        )
        ...
        if hasattr(worker, "worker_thread") and worker.worker_thread is not None:
            self.assertFalse(
                worker.worker_thread.is_alive(), "Worker thread should not be alive."
            )
    ...
    def test_request_best_move(self, mock_popen_uci_success):
        ...
        mock_engine_instance.play = MagicMock(
            return_value=MagicMock(move=chess_stub.Move.from_uci(expected_move_uci))
        )
        ...
        board = DummyBoard()
        returned_move = worker.request_best_move(
            board, time_limit=0.01
        ...
        self.assertEqual(
            returned_move.uci(),
            expected_move_uci,
            f"Returned move UCI should be {expected_move_uci}.",
        )
        ...
        self.assertEqual(
            worker.get_state(),
            EngineState.IDLE,
            "Engine should return to IDLE after finding move.",
        )
