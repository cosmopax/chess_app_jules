...
class TestMainWindow(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        mock_qt_widgets.QWidget = type(
            "MockQWidget", (MagicMock,), {"__module__": "PySide6.QtWidgets"}
        )
        ...
        mock_qt_widgets.QFileDialog = MagicMock(
            getOpenFileName=MagicMock(
                name="MockQFileDialog_getOpenFileName_static_method"
            ),
            getSaveFileName=MagicMock(
                name="MockQFileDialog_getSaveFileName_static_method"
            ),
        )
        ...
        def create_mock_timer_instance(*args, **kwargs):
            ...
            instance.timeout.connect = MagicMock(
                name="MockQTimer_Instance_timeout_connect"
            )
        ...
        MockQTimerClass = MagicMock(
            name="MockQTimer_ClassFactory", side_effect=create_mock_timer_instance
        )
        mock_qt_core.QTimer = MockQTimerClass
        mock_qobject_attrs = {
            "__module__": "PySide6.QtCore",
            "setObjectName": MagicMock(),
        }
        ...
        mock_qt_core.Signal = MagicMock(
            name="MockSignalClassFactory", return_value=mock_signal_instance
        )
        ...
        mock_qt_svg_widgets.QSvgWidget = type(
            "MockQSvgWidgetBase", (MagicMock,), {"__module__": "PySide6.QtSvgWidgets"}
        )
        ...
        mock_chess_module.STARTING_FEN = (
        ...
        mock_chess_pgn_module.Game = MagicMock(
            return_value=MagicMock(name="MockPgnGameInstance")
        )
    ...
    def tearDownClass(cls):
        ...
        modules_to_delete = [
            "chess_app.ui.main_window",
            "chess_app.ui.chess_clock",
            "chess_app.engine.engine_worker",
            "chess_app.openings",
        ]
    ...
    def setUp(self):
        ...
        self.qapplication_patch = patch(
            "chess_app.ui.main_window.QApplication",
            MagicMock(name="MockQApplication_setUp"),
        )
        self.engineworker_patch = patch(
            "chess_app.ui.main_window.EngineWorker",
            MagicMock(name="MockEngineWorker_setUp"),
        )
        self.chessclock_patch = patch(
            "chess_app.ui.main_window.ChessClock",
            MagicMock(name="MockChessClock_setUp"),
        )
        ...
        self.reset_board_patcher = patch.object(
            CurrentMainWindow, "reset_board", MagicMock(name="PatchedResetBoard")
        )
        self.mock_reset_board_method = self.reset_board_patcher.start()

        with patch.object(
            sys.modules["chess"].svg,
            "board",
            MagicMock(
                return_value=MagicMock(
                    encode=MagicMock(return_value=b"<svg>test svg</svg>")
                )
            ),
        ), patch.object(CurrentMainWindow, "_init_ui", MagicMock()), patch.object(
            CurrentMainWindow, "_create_menu", MagicMock()
        ), patch.object(
            CurrentMainWindow, "update_ui_from_engine_state", MagicMock()
        ):
            self.main_window = CurrentMainWindow(engine_path="dummy_engine")

        sys.modules["chess"].svg.board.return_value.encode.return_value = (
            b"<svg>runtime</svg>"
        )
        ...
        self.main_window.status_label.setText = MagicMock(
            name="MockStatusLabel_setText_GlobalSetup"
        )
        ...
        self.main_window.start_analysis_button = MagicMock(
            name="MockStartAnalysisButton"
        )
        self.main_window.reset_board_button = MagicMock(name="MockResetBoardButton")
        self.main_window.prev_mainline_move_button = MagicMock(
            name="MockPrevMainlineButton"
        )
        self.main_window.next_mainline_move_button = MagicMock(
            name="MockNextMainlineButton"
        )
        ...
        if hasattr(self.main_window, "board_display") and hasattr(
            self.main_window.board_display, "player_color"
        ):
    ...
    @patch(
        "chess_app.ui.main_window.QMessageBox"
    )  # Note: Order of decorators means mock_qmessagebox is first arg
    def test_toggle_analysis_start_stop(
        self, mock_qmessagebox, mock_logger, mock_process_events
    ):
        # EngineState is globally available from setUpClass

        # Part 1: Test starting analysis
        self.main_window.engine_worker.get_state = MagicMock(
            return_value=EngineState.IDLE
        )
        self.main_window.board.copy = MagicMock(return_value=self.main_window.board)
        self.main_window.engine_worker.engine = (
            MagicMock()
        )  # Ensure guard clause passes
        ...
        if hasattr(self.main_window, "update_ui_from_engine_state") and hasattr(
        ...
        with patch.object(
            self.main_window,
            "status_label",
            MagicMock(name="status_label_runtime_patch"),
        ), patch.object(
            self.main_window, "update_ui_from_engine_state"
        ) as mock_update_ui_method_runtime:  # Patching the instance method
            ...
            self.main_window.engine_worker.start_analysis.assert_called_once_with(
                self.main_window.board
            )
        ...
        self.main_window.engine_worker.get_state = MagicMock(
            return_value=EngineState.ANALYZING
        )
        ...
        with patch.object(
            self.main_window, "status_label", MagicMock(name="status_label_stop_patch")
        ), patch.object(
            self.main_window, "update_ui_from_engine_state"
        ) as mock_update_ui_method_runtime_stop:
    ...
    def test_request_engine_move(self, mock_qmessagebox):
        ...
        if not hasattr(self.main_window, "status_label") or not isinstance(
        ...
        self.main_window.engine_worker.request_best_move = MagicMock(
            return_value=mock_move
        )
        self.main_window.engine_worker.engine = MagicMock()

        with patch.object(
            self.main_window, "handle_engine_move"
        ) as mock_handle_engine_move, patch.object(
            self.main_window, "update_ui_elements"
        ), patch.object(
            self.main_window, "update_ui_from_engine_state"
        ):
            ...
            expected_status_text = (
                f"Status: Engine ({self.main_window.engine_path}) is thinking..."
            )
    ...
    @patch(
        ...
        read_data="rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1",
    )
    def test_load_fen_from_file_ok(
        self, mock_file_open, mock_qmessagebox, mock_qfiledialog_getname_method
    ):
        mock_qfiledialog_getname_method.return_value = (
            "/fake/path.fen",
            "FEN Files (*.fen)",
        )
        with patch.object(self.main_window, "load_fen") as mock_load_fen:
            ...
            mock_file_open.assert_called_once_with(
                "/fake/path.fen", "r", encoding="utf-8"
            )
            mock_load_fen.assert_called_once_with(
                "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"
            )
    ...
    def test_handle_game_over_checkmate(self, mock_qmessagebox):
        ...
        mock_qmessagebox.information.assert_called_once_with(
            self.main_window, "Game Over", "Checkmate! Black wins."
        )
        self.main_window.status_label.setText.assert_any_call(
